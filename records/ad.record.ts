import {AdEntity, NewAdEntity, SimpleAdEntities} from "../types";
...
export class AdRecord implements AdEntity {
    ...
    constructor(obj: NewAdEntity) {
        ...
        if(obj.price < 0 || obj.price > 9999999){
            throw new ValidationError('Cena nie moze być mniejsza niż 0 lub większa niż 9 999 999.');
        }
    ...
    static async findAll(name: string): Promise<SimpleAdEntities[]> {
    ...
    async insert(): Promise<void> {
        if(!this.id){
            ...
            throw new Error('Cannot insert semething thet is already inserted!');
        }
